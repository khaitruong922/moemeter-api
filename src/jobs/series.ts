...
export const syncBookSeries = async (
	...
	for (const book of books) {
		...
		const result = await bookmeterApiService.fetchBookSeries(book.id);

		if (result === null) {
			await markSeriesFetched(sql, [book.id]);
		...
		const allBookIds = [...new Set([book.id, ...result.books.map((b) => b.id)])];
