...
app.get('/users/:id/summary/monthly/:year/:month', async (c) => {
	...
	const jsonBooks = await getJsonSummaryMonthly(
		`https://bookmeter.com/users/${id}/summary/monthly/${year}/${month}`,
		{
			perPage,
			reqPage,
			isAsc,
...
app.get('/users/:id/summary/yearly/:year', async (c) => {
	...
	const jsonBooks = await getJsonSummaryYearly(
		`https://bookmeter.com/users/${id}/summary/yearly/${year}`,
		{
			perPage,
			reqPage,
			isAsc,
