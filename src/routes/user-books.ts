...
import { DEFAULT_BOOKS_LIMIT, isAscQuery } from '../utils/bookmeter-utils';
...
app.get('/users/:id/books/read', async (c) => {
  ...
  const perPage = applyNaNVL(parseNatNum(c.req.query('limit')), DEFAULT_BOOKS_LIMIT);
	const reqPage = applyNaNVL(parseNatNum(c.req.query('page')), 1);
	const isAsc = isAscQuery(c.req.query('order'));
